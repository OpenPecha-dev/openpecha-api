...
def get_hfml_text(text_id, opf_path):
...
def get_page_obj(page, text_meta, tag, pagination_layer):
    ...
    if get_clean_page(page_content) == "\n":
        ...
        if tag == "note":
            page_obj = NotesPage(id=page_id, page_no=pg_num, content=page_content, name= f"Page {pg_num}", vol=text_meta['vol'], image_link=page_link)
        else:
            page_obj = Page(id=page_id, page_no=pg_num, content=page_content, name= f"Page {pg_num}",vol=text_meta['vol'], image_link=page_link, note_ref=note_ref)
...
def get_text_obj(hfmls, text_meta, opf_path):
    pages = []
    notes = []
    for vol_num, hfml_text in hfmls.items():
        text_meta['vol'] = int(vol_num[1:])
        pagination_layer = from_yaml(Path(f"{opf_path}/{text_meta['pecha_id']}.opf/layers/v{int(text_meta['vol']):03}/Pagination.yml"))
